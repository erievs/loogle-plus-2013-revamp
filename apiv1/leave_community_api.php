...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($request_params['community_id']) && isset($request_params['username'])) {
        ...
        if ($conn->connect_error) {
            ...
            $query = "UPDATE communities SET members_list = REPLACE(members_list, ?, ''), members = members - 1 WHERE community_id = ?";
            ...
            if ($stmt->execute()) {
                ...
                $response['message'] = 'You have successfully left the community';
            } else {

                $response['status'] = 'error';
                $response['message'] = 'Failed to leave the community';
            }
